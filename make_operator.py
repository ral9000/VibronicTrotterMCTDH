...
def coupling_array_to_sparse_polynomial(coupling_array, position_operators):
    ...
    if M != len(position_operators):
        raise ValueError(
            f"Expected M={M} from the shape of `coupling_array`, instead got {len(position_operators)} position operators!"
        )
...
def make_population_observable(i, n_states, n_modes, n_grid):
    ...
    blocks = [
        vib_identity if idx == i else csr_matrix((n_grid**n_modes, n_grid**n_modes))
