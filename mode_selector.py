...
def rank_modes(omegas, couplings, ranking_type='C1N'):
    ...
    valid_types = ['C1N', 'C2N', 'F', 'C1NDIVF', 'C1NMULF', 'C1N+F']

    if ranking_type.upper() not in valid_types:
        raise ValueError(f'Ranking type {ranking_type} not recognized. Must be one of: {valid_types}')
    ...
    for mode_idx in range(len(omegas)):
        ...
        for coupling_arr in non_constant_couplings: 
            if len(np.shape(coupling_arr)) == 3: #linear term
                couplings = coupling_arr[:,:,mode_idx].flatten()
    ...
    for mode_idx in mode_interactions:
        if ranking_type.upper() == 'C1N': 
            ...
            mode_scores[mode_idx] = mode_pnorm(mode_interactions[mode_idx], p = 1, omega=omegas[mode_idx], freq_usage='penalty')
        elif ranking_type.upper() == 'C1NMULF':
            mode_scores[mode_idx] = mode_pnorm(mode_interactions[mode_idx], p = 1, omega=omegas[mode_idx], freq_usage='bonus')
        elif ranking_type.upper() == 'C1N+F':
            mode_scores[mode_idx] = mode_pnorm(mode_interactions[mode_idx], p = 1) + abs(omegas[mode_idx])
...
def truncate_by_states(coupling_arrays, states_to_keep):
    ...
    for idx, coupling_arr in coupling_arrays.items():
        ...
        if dim == 2: #0th order in vib
            truncated_cpl_arr = coupling_arrays[idx][np.ix_(states_to_keep, 
                                                           states_to_keep)]
...
def get_truncated_coupling_arrays(coupling_arrays, selected_modes, selected_states=None, only_diagonal=True):
    ...
    for idx, coupling_arr in coupling_arrays.items():
        if idx == 1: #linear terms 
            ...
            if only_diagonal:
                subcoupling_arr_quad = np.stack([coupling_arr[:, :, i, i] for i in selected_modes], axis=-1)  
            else:
                subcoupling_arr_quad = coupling_arr[:, :, selected_modes, :][:, :, :, selected_modes]  # Shape (N, N, M', M')
