...
def effective_subspace_projector(dimension, threshold=1e-9):
    vals, vecs = eigh(
        (
            1j * commutator(position_operator(dimension, 1), momentum_operator(dimension, 1))
        ).todense()
    )
...
def find_num_steps(
    actual, fragments, total_time, epsilon, initial_guess=10, norm_type="spectral", projector=None
):
    """
...
def find_num_steps_expectation_dependent(
    actual, fragments, total_time, epsilon, state, observables, initial_guess=10
):
    """
    ...
    while True:
        estimated = trotterize(fragments, total_time, n_steps)
        errors = [
            expectation_value_error(U_exact=actual, U_approx=estimated, psi=state, O=observable)
...
def find_num_steps_observable_spectral(
    exact_propagator,
    h_fragments,
    total_time,
    epsilon,
    ...
    while True:
        ...
        errors = [
            compute_observable_spectral_norm_error(exact_propagator, estimated, O, projector)
...
def r_trotter_steps_from_exact_spectral(
    total_time, epsilon, exact_propagator, h_fragments, init_guess
):
    t0 = time()
    r = find_num_steps(
        exact_propagator, h_fragments, total_time, epsilon, init_guess, norm_type="spectral"
    )
    print(
        f"Found r = {r} for EXACT_SPECTRAL w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_spectral_projected(
    total_time, epsilon, exact_propagator, h_fragments, projector, init_guess
):
    t0 = time()
    r = find_num_steps(
        exact_propagator,
    ...
    print(
        f"Found r = {r} for EXACT_SPECTRAL_PROJECTED w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r
...
def r_trotter_steps_from_exact_commutator_spectral(total_time, epsilon, h_fragments):
    ...
    print(
        f"Found r = {r} for EXACT_COMMUTATOR_SPECTRAL w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_trace_norm(
    total_time, epsilon, exact_propagator, h_fragments, init_guess
):
    t0 = time()
    r = find_num_steps(
        exact_propagator, h_fragments, total_time, epsilon, init_guess, norm_type="trace"
    )
    print(
        f"Found r = {r} for EXACT_TRACE w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_trace_norm_projected(
    total_time, epsilon, exact_propagator, h_fragments, projector, init_guess
):
    t0 = time()
    r = find_num_steps(
    ...
    print(
        f"Found r = {r} for EXACT_TRACE_PROJECTED w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_state_norm(
    total_time, epsilon, exact_propagator, h_fragments, state, init_guess
):
    t0 = time()
    r = find_num_steps_state_dependent(
        exact_propagator, h_fragments, total_time, epsilon, state, initial_guess=init_guess
    )
    print(
        f"Found r = {r} for EXACT_STATE w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_expectationvalue(
    total_time, epsilon, exact_propagator, h_fragments, state, observables, init_guess
):
    t0 = time()
    r = find_num_steps_expectation_dependent(
        exact_propagator,
        h_fragments,
        total_time,
        epsilon,
    ...
    print(
        f"Found r = {r} for EXACT_EXPECTATION w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_observable_spectral(
    total_time, epsilon, exact_propagator, h_fragments, observables, init_guess
):
    """
    || U_trot' O U_trot - U' O U   ||
    """
    t0 = time()
    r = find_num_steps_observable_spectral(
        exact_propagator,
        h_fragments,
        total_time,
        epsilon,
    ...
    print(
        f"Found r = {r} for EXACT_OBSERVABLE_SPECTRAL w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def r_trotter_steps_from_exact_projected_observable_spectral(
    total_time, epsilon, exact_propagator, h_fragments, observables, projector, init_guess
):
    """
    ...
    print(
        f"Found r = {r} for EXACT_OBSERVABLE_SPECTRAL_PROJECTED w/ t={total_time}, error < {epsilon} after {round(time() - t0, 2)} seconds "
    )
    return r


def compute_observable_error(
    observable, exact_propagator, trotter_propagator, projector=None, norm_type="spectral"
):
    """
