...
def generate_parameter_section(param_dict, units='ev'):
    ...
    for label in param_dict: 
        ...
        val = np.format_float_positional(param_dict[label], precision=20, unique=False, fractional=True, trim='k')
        shortened_label = short_label(label)
        if shortened_label in param_dict_shortened: #this label has been used before!
            print(f'WARNING : Short label {shortened_label} (longform={label}) has already been used. Adding a zero.')
            shortened_label += '0'
