...
def check_convergence(dir, name, spf_range, n_states, ctol = 0.1, plotting=True):
    ...
    for idx, n_spf in enumerate(spf_range): 
        ...
        if n_spf == 'exact':
                    errors, standard_pops, effective_pops = get_errors(dir, system=f'{name}_exact', n_states=n_states, return_pop_series=True)
        else:
            errors, standard_pops, effective_pops = get_errors(dir, system=f'{name}_spf{n_spf}', n_states=n_states, return_pop_series=True)
        ...
        if idx > 0:
            ...
            for ti in range(len(curr_std)):
                d_std = sum([(curr_std[ti][idx] - prev_std[ti][idx])**2 for idx in range(n_states)])
                d_eff = sum([(curr_eff[ti][idx] - prev_eff[ti][idx])**2 for idx in range(n_states)])
                conv_err_std += d_std 
                conv_err_eff += d_eff 
            conv_err_std = np.sqrt(conv_err_std)
            conv_err_eff = np.sqrt(conv_err_eff)

            print(f'cumulative improvement going from n_spf={idx-1} to n_spf={idx} in observables (std): {conv_err_std}')
            print(f'cumulative improvement going from n_spf={idx-1} to n_spf={idx} in observables (eff): {conv_err_eff}\n')

            if conv_err_std < ctol and conv_err_eff < ctol:
                print(f'Population converged at std: {round(conv_err_std,4)}, eff: {round(conv_err_eff,4)}')
    ...
    print(f'Did not find convergence (ctol = {ctol}) by n_spf = {spf_range[-1]}\nstd rmse: {round(conv_err_std,4)} \neff rmse: {round(conv_err_eff,4)}')
...
def get_run_pops(dir, name, n_states):
    ...
    is_conv, rmses, pops = check_convergence(dir, name, spf_range, n_states, ctol = 0.1, plotting=True)
...
def read_series(dir, name, m_set, max_spf, n_states):
    ...
    for m in m_set:
        ...
        n_spf_highest, pop_series, conv_errs = get_highest_spf_run(dir, name_m, range(1,max_spf+1), n_states)
        print(f'Using the n_spf = {n_spf_highest} results for m = {m} (convergence errors = {conv_errs}).')
...
def get_error_metrics(std_popseries, eff_popseries, n_states, t_max=None):
    ...
    errors_total =[np.sum([errors[t][i] for t in errors.keys() if t <= t_max]) for i in range(n_states)] 
    errors_mean = [np.average([errors[t][i] for t in errors.keys() if t <= t_max]) for i in range(n_states)]
    errors_var = [np.var([errors[t][i] for t in errors.keys() if t <= t_max]) for i in range(n_states)]
...
if __name__ == "__main__":
    ...
    for deltaT in deltaT_series: 
        for m in mode_series: 
            ...
            spf_range = [8]
            is_conv, conv_errs, pops  = check_convergence(dir, name, ['exact'], n, ctol = 0.1, plotting=True)
            ...
            for idx in range(n):
                axes[0].plot(list(std_pops.keys()), [e[idx] for e in std_pops.values()], label=f'd{idx+1}', color=d_colors[idx], linestyle=d_styles[idx])
                axes[1].plot(list(eff_pops.keys()), [e[idx] for e in eff_pops.values()], label=f'd{idx+1}', color=d_colors[idx], linestyle=d_styles[idx])
                axes[2].plot(list(errors.keys()), [e[idx] for e in errors.values()], label=f'd{idx+1}', color=d_colors[idx], linestyle=d_styles[idx])
            ...
            errors_total_100fs, errors_mean_100fs, errors_var_100fs, errors_max_100fs, errors_at_finalt_100fs = errors_100fs 
            errors_total_1000fs, errors_mean_1000fs, errors_var_1000fs, errors_max_1000fs, errors_at_finalt_1000fs = errors_1000fs
